...
namespace CommonUtil
{
    ...
    public class EncryptUtil
    {
        ...
        /// 密钥存储路径，放在user目录下。
        /// </summary>
        static string rootFolderPath = rootPath + "\\Huawei\\SCCM Plugin";
        //密钥组件分散保存，当密钥组件存储于文件中时，不能所有的组件文件放在同一目录下存储；
        //因此加多一层目录.
        static string rootFile = rootPath + "\\Huawei\\SCCM Plugin\\base.kn";
        static string mainFile = rootPath + "\\Huawei\\SCCM Plugin\\main\\main.kn";
        /// <summary>
        /// 获得根密钥
        /// </summary>
        /// <returns>根密钥</returns>
        ...
        )
        {
            ...
            /*
             * 2017-10-09 Using ssl dll to gen the password.
           */
            /*
             参数说明：
            ...
            PKCS5_PBKDF2_HMAC(passPhrase, 16, saltValueBytes, 16, passwordIterations, EVP_sha256(), 32, keyBytes);
        ...
        public static string GetRootKeyFromPath(bool isInit)
        {
          ...
          //+BitConverter.ToString(Encoding.Unicode.GetBytes(plainText)).Replace("-", String.Empty));
           ...
           // string hashAlgorithm = "SHA256";             // can be "MD5"
            ...
            string trueEnKey = GenRootKey(plainText, saltValue, passwordIterations, keySize);
        ...
        #region 兼容旧的密钥
        static string versionFile = rootPath + "\\Huawei\\SCCM Plugin\\vrn.kn";
